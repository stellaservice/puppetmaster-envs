...
function ux_theme_meta_save($post_id) {  
	...
	if($thumbnail_id){
		...
		if(!$thumbnail_crop){
			ux_theme_option_process_generate_thumb($thumbnail_id);
		}
	}
    ...
    <li class="commlist-unit">
        ...
        <div class="comm-u-wrap">
        	...
        	<div class="comment-meta">
                ...
                <span class="date"><?php echo human_time_diff(get_comment_time('U'), current_time('timestamp'));  esc_html_e(" ago","ux"); ?></span>
                <span class="reply"><?php comment_reply_link(array_merge( $args, array('reply_text'=>esc_html__('Reply','ux'),'depth' => $depth, 'max_depth' => $args['max_depth']))) ?></span>
		...
		<?php if ($comment->comment_approved == '0'){ ?>
            <p><em><?php esc_html_e('Your comment is awaiting moderation','ux'); ?>.</em></p>
        <?php } ?>
    </li>				
<?php 
}
