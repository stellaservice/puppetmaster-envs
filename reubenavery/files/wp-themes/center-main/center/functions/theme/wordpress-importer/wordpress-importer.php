...
class WP_Import extends WP_Importer {
	...
	function import_end() {
		...
		echo '<p>' . __( 'All done.', 'wordpress-importer' ) . ' <a href="' . admin_url() . '" data-success>' . __( 'Have fun!', 'wordpress-importer' ) . '</a>' . '</p>';
...
<form action="<?php echo admin_url( 'admin.php?import=wordpress&amp;step=2' ); ?>" method="post">
	...
	<input type="hidden" name="import_id" value="<?php echo sanitize_text_field($this->id); ?>" />
...
<?php
	 ...
	 * @return int 60
	 */
	function bump_request_timeout($time = 240) {
		return $time;
	}
